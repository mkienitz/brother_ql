//! Raster image processing for Brother QL printers

use image::{DynamicImage, GenericImageView, GrayImage, ImageBuffer, Rgb, imageops::{self, BiLevel}};

use crate::{error::PrintJobError, media::Media};

pub(crate) type RasterLayer = Vec<[u8; 90]>;
...
impl RasterImage {
    pub(crate) fn new(img: DynamicImage, media: Media) -> Result<Self, PrintJobError> {
        ...
        if media.width_dots() != width {
            return Err(PrintJobError::dimension_mismatch(
                media.width_dots(),
                width,
                None,
                height,
        ...
        // For die-cut labels, also check height
